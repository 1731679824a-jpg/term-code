...
from termcode.config import ConfigError, load_config
from termcode.hooks import HookConfigError, HookEngine, load_hooks
from termcode.permissions import PermissionMode


def main() -> None:
    # 先确保 .termcode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".termcode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".termcode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="termcode", description="TermCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from termcode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from termcode.agent import Agent
    from termcode.client import create_client, resolve_context_window
    from termcode.conversation import ConversationManager
    from termcode.memory.instructions import load_instructions
    ...
    from termcode.tools import create_default_registry
