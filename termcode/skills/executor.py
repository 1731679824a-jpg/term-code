...
from termcode.skills.parser import SkillDef, substitute_arguments
from termcode.tools import ToolRegistry

if TYPE_CHECKING:
    from termcode.agent import Agent, AgentEvent
...
class SkillExecutor:
    ...
    async def execute_fork(
        ...
        from termcode.agent import Agent as AgentClass, StreamText, LoopComplete, ErrorEvent
    ...
    def _register_skill_tools(self, skill: SkillDef) -> None:
        # 目录型 skill 可以在 tool.json + references/ 中携带专用工具，
        # 在执行前注册到 agent 的 registry（重复注册会被跳过）。
        if not skill.is_directory or skill.source_path is None:
            return
        from termcode.skills.directory import register_skill_tools
