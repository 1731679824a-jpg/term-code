...
from termcode.worktree.models import WorktreeSession
...
def _session_path(termcode_dir: Path) -> Path:
    return termcode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(termcode_dir)
...
def load_worktree_session(termcode_dir: Path) -> WorktreeSession | None:
    path = _session_path(termcode_dir)
    if not path.exists():
        return None
    try:
