...
from termcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from termcode.conversation import ThinkingBlock as ConvThinkingBlock
from termcode.memory.auto_memory import MemoryManager
from termcode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from termcode.hooks import HookContext, HookEngine, ToolRejectedError
from termcode.hooks.engine import HookNotification
from termcode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from termcode.tools import ToolRegistry
from termcode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".termcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".termcode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from termcode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from termcode.context.manager import (
