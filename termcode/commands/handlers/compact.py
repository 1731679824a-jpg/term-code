...
from termcode.commands.registry import Command, CommandContext, CommandType
...
async def handle_compact(ctx: CommandContext) -> None:
    ...
    from termcode.agent import CompactNotification, ErrorEvent
    ...
    if isinstance(result, CompactNotification):
        ...
        if ctx.session is not None and result.boundary is not None:
            from termcode.memory.session import make_compact_boundary
