...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".termcode" / "config.yaml",
        cwd / ".termcode" / "config.yaml",
        cwd / ".termcode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .termcode/config.yaml "
            "in project or ~/.termcode/config.yaml"
        )
    return merged
